...
段落元素与标题元素只能嵌套内联元素 就是p标签和h系列标签只能嵌套行内元素
...
背景图多为图标等颜色比较简单、文件体积不大、起**修饰作用**的图片
...
### ClassName命名    
ClassName的命名应该尽量精短、明确，必须以**字母开头命名**，且**全部字母为小写**，单词之间**统一使用下划线** “_” 连接   user_name
